...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if APP_TYPE == 1:
            self.agent = RLAgent(state_space_size=100, action_space=[0, 1])  # Adjust state space size
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        if flags == 0:
            ...
            if APP_TYPE == 1:
                ...
                self.agent.track_metrics(action, self.mitigation, self.mitigation)
    ...
    def _get_state(self, sfe, ssip, rfip):
        ...
        sfe_norm = min(sfe / 20, 1.0)
        ssip_norm = min(ssip / 10, 1.0)
        rfip_norm = min(rfip, 1.0)
    ...
    def _calculate_reward(self, action):
        ...
        if action == 1 and self.mitigation:
            return 10  # Reward for successful mitigation
        elif action == 0 and not self.mitigation:
            return -5  # Reward for correctly allowing normal traffic
        elif action == 0 and self.mitigation:  # Incorrectly allowed an attack
            return -10
        else:
            return 5 
